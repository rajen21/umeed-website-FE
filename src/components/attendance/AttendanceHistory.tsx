...
export function AttendanceHistory() {
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | "student" | "volunteer">(
    "all",
  );
  ...
  const { data: records, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const studentRecords: AttendanceRecord[] = (studentsData as any[]).map(
        (r: any) => ({
          id: r.id,
      ...
      const volunteerRecords: AttendanceRecord[] = (
      ...
      return [...studentRecords, ...volunteerRecords].sort(
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4">
        ...
        <StatsCard
          ...
          value={`${stats.rate}%`}
          className={stats.rate > 80 ? "text-green-600" : "text-amber-600"}
        />
  ...
  return (
    <Card>
      <CardContent className="p-3 md:p-6 flex flex-col items-center justify-center text-center gap-1">
        <span className="text-[10px] md:text-sm font-medium text-muted-foreground uppercase tracking-wider leading-tight">
          {label}
        </span>
        <span
          className={cn(
