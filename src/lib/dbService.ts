...
class QueryBuilder<T> {
  ...
  select(
    ...
    if (options?.head) {
      this.headOnly = true; // ⭐ ADD THIS LINE
    }
    return this;
  }

  log() {
    console.log(this.orderBy, this.limitValue, this.selectColumns);
  }
  ...
  filter(column: string, operator: string, value: any): this {
    // Map Supabase-style operators to your internal operators
    const opMap: Record<string, Filter["operator"]> = {
      eq: "eq",
      neq: "neq",
      gt: "gt",
      gte: "gte",
    ...
    try {
      ...
      if (this.operation === "select") {
        ...
        this.filters.forEach((f) => {
          if (f.operator === "eq") {
            if (f.value === null) {
              params.append(f.column, "null"); // backend should treat as IS NULL
            } else {
              params.append(f.column, String(f.value));
            }
          }
        ...
        if (idFilter) {
          ...
          if (this.payload.id) {
            ...
            console.warn(
              "Update without ID filter or ID in payload might fail",
            );
          }
        }
      ...
      if (this.isSingle && Array.isArray(data)) {
        return { data: data[0] || null, error: null, count: 0 };
      }
