...
class AuthService {
    ...
    try {
      ...
      if (stored) {
        ...
        if (isValidToken) {
          ...
          console.warn(
            "[Auth] Invalid or legacy token detected, clearing session",
          );
  ...
  async changePassword({
    ...
    try {
      ...
      if (!response.ok) {
        return {
          data: null,
          error: { message: data.error || "Change password failed" },
        };
      }
  ...
  async updateUser(data: { data?: Record<string, number | string> }): Promise<{
    data: { user: User } | null;
    error: { message: string } | null;
  }> {
    try {
      ...
      if (!response.ok) {
        return {
          data: null,
          error: { message: resData.error || "Update failed" },
        };
      }
      ...
      if (this.currentSession && this.currentSession.user) {
        ...
        this.currentSession.user.user_metadata =
          resData.user_metadata || this.currentSession.user.user_metadata;
        this.currentSession.user.preferences =
          resData.preferences || this.currentSession.user.preferences;
